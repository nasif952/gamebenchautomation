...
last_mouse_pos = (0, 0)
in_game_mode = False
MOVE_THRESHOLD = 5  # Min distance to record movement (reduce noise)
...
def poll_mouse_position():
    ...
    while recording:
        ...
        dist = (dx**2 + dy**2)**0.5
        if dist >= MOVE_THRESHOLD:  # Only record significant movements
            if not in_game_mode:
            ...
            last_x, last_y = x, y
        last_mouse_pos = (x, y)
        time.sleep(0.05)  # Slower polling to reduce jitter
