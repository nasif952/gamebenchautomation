...
last_mouse_pos = (0, 0)  # Track last position for relative movement
...
def poll_mouse_position():
    ...
    while recording:
        ...
        if (x, y) != (last_x, last_y):
            ...
            events.append(("move_absolute", x, y, elapsed_time))  # Still record absolute for menus
            print(f"Mouse moved to ({x}, {y}) at {elapsed_time:.2f}s")
            last_x, last_y = x, y
        last_mouse_pos = (x, y)  # Update last known position
...
def on_click(x, y, button, pressed):
    ...
    elapsed_time = time.time() - start_time
    click_x, click_y = last_mouse_pos  # Use polled position
    if pressed:
