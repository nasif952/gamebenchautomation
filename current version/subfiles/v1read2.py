...
last_mouse_pos = (0, 0)
in_game_mode = False
MOVE_THRESHOLD = 0.1  # Even lower for finest movements
...
def poll_mouse_position():
    ...
    while recording:
        ...
        dist = (dx**2 + dy**2)**0.5
        if dist >= MOVE_THRESHOLD:  # Record even tiny movements
            if not in_game_mode:
            ...
            last_x, last_y = x, y
        last_mouse_pos = (x, y)
        # Sleep to target ~142 Hz, accounting for loop time
        elapsed_loop = time.time() - loop_start
        sleep_time = max(0, 0.007 - elapsed_loop)  # ~7ms per poll
