...
last_mouse_pos = (0, 0)
in_game_mode = False
MOVE_THRESHOLD = 0.5  # Lowered to capture finer movements
...
def poll_mouse_position():
    ...
    while recording:
        ...
        dist = (dx**2 + dy**2)**0.5
        if dist >= MOVE_THRESHOLD:  # Record significant movements
            if not in_game_mode:
            ...
            last_x, last_y = x, y
        last_mouse_pos = (x, y)
        time.sleep(0.007)  # ~142 Hz polling to match mouse rate (100-140 Hz)
